...
//        constant.setConstantValue(withUnsafeBytes(of: 0.032 as Float32, \.baseAddress.unsafelyUnwrapped), type: .float, index: 1) // k
            ...
            let cs = MTL4SpecializedFunctionDescriptor()
            cs.constantValues = constant
            let ss = MTL4LibraryFunctionDescriptor()
            ss.name = "gscs"
            ss.library = library
            cs.functionDescriptor = ss
            ckDescriptor.computeFunctionDescriptor = cs
        }
        ...
        let cancel = signal.sink {
            switch $1 {
            case is TapGesture:
                texture.replace(region: .init(origin: .init(x: .init($0.x), y: .init($0.y), z: 0), size: .init(width: 32, height: 32, depth: 1)),
                                mipmapLevel: 0, slice: 0,
                                withBytes: Array<Float32>(repeating: 0.00, count: 32 * 32),
                                bytesPerRow: MemoryLayout<Float32>.stride * 32,
                                bytesPerImage: MemoryLayout<Float32>.stride * 32 * 32)
                texture.replace(region: .init(origin: .init(x: .init($0.x), y: .init($0.y), z: 0), size: .init(width: 32, height: 32, depth: 1)),
                                mipmapLevel: 0, slice: 1,
